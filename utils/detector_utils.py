...
class WebcamVideoStream:
    def __init__(self, src, width, height):
        ...
        # from the stream
#        if "http" in src:
#            imageRes = urllib.request.urlopen(src)
#            frame = np.array(bytearray(imageRes.read()))
#            self.stream.set(cv2.imdecode(frame, 1))
#        else:	
...
class IPWebcamVideoStream:
    def __init__(self, src, width, height):
        ...
        imageRes = urllib.request.urlopen(self.src)
        frame = np.array(bytearray(imageRes.read()))
